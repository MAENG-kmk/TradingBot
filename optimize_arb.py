...
def score(r) -> float:
    if r is None or r['total'] < MIN_TRADES:
        return -999.0
    trade_bonus = np.sqrt(r['total'] / 100)
    return r['sharpe'] * trade_bonus
...
if __name__ == '__main__':
    ...
    # ── Stage1 이미 완료된 경우 결과 고정 (재실행 방지) ─────────────
    # best1: bb_period=20, bb_std=1.5, rsi_overbuy=80, rsi_oversell=20, adx_threshold=10

    # Stage1 최적값 반영 (int 파라미터 명시 변환)
    INT_PARAMS = {'bb_period', 'rsi_overbuy', 'rsi_oversell', 'adx_threshold'}
    s1_best_vals = {k: (int(best1[k]) if k in INT_PARAMS else float(best1[k]))
                    for k in s1_keys}
    stage2_base = {**BASE_CFG, **s1_best_vals}
    ...
    # ── 최종 결과 ────────────────────────────────────────────────────
    final_cfg = {**stage2_base,
                 **{k: best2[k] for k in s2_keys}}

    print(f"\n{'='*60}")
    print("  최종 최적 파라미터 (coins/arb/strategy.py 반영용)")
    print(f"{'='*60}")
    param_map = {
